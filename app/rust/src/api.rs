...
pub use self::storage::storage_loader::StorageInstance;
...
#[derive(Clone)]
pub struct DownloadProgress(Arc<DashMap<CollectionId, Progress>>);

impl DownloadProgress {
    pub fn get_p(&self) -> Vec<(CollectionId, Progress)> {
        let a = (&*self.0.clone()).clone();
...
lazy_static::lazy_static! {
    ...
    pub static ref DOWNLOAD_PROGRESS: DownloadProgress = DownloadProgress(Arc::new(DashMap::default()));
